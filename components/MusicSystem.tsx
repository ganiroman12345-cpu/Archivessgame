...
export const MusicSystem: React.FC = () => {
    ...
    if (!loop) {
      audio.onended = () => {
        ...
        const handleStopAudioEnd = () => {
          ...
          if (stillMultiplayer) {
            // Alternate between cybernetic and power!
            const nextTrack = currentTrackSrcRef.current?.includes('cybernetic')
              ? '/audio/power.mp3'
  ...
  useEffect(() => {
    ...
    if (isMultiplayerScreen) {
      // If already playing one of the multiplayer tracks, keep it playing across screens
      if (
        currentTrackSrcRef.current && 
        (currentTrackSrcRef.current.includes('cybernetic') || currentTrackSrcRef.current.includes('power')) &&
        lastModeRef.current === 'MULTIPLAYER' &&
        isPlayingRef.current
      ) {
        // Continue current track
      } else {
        const randomTrack = Math.random() < 0.5 ? '/audio/cybernetic.mp3' : '/audio/power.mp3';
